...
def train_one_epoch_enhanced(model, optimizer, optimizer2, data_loader, device, epoch):
    """
    Train enhanced ATOM model for one epoch
    
    Args:
        model: Enhanced ATOM model instance
    ...
    """
    model.train()
    # If using BrainMVP, set it to evaluation mode
    if model.use_brainmvp and model.brainmvp_extractor is not None:
        model.brainmvp_extractor.eval()
    ...
    for step, data in enumerate(data_loader):
        ...
        with autocast():
            # Forward pass using enhanced model
            (pred1, pred_class, pred_class_k1, x_set), (pred_new1, pred_class_new, pred_class_k1_new) = model(images3d, images2d)
        
        # First stage loss calculation
        ...
        # Process prediction results to find correct blocks
        next_block_list = []
        position_list = []
        pred_class_list = []
        
        # [Original processing logic, keeps unchanged]
        
        # Second stage loss calculation
        ...
        # Gradient clipping
        torch.nn.utils.clip_grad_norm_(parameters=model.vit_2d.parameters(), max_norm=10, norm_type=2)
        torch.nn.utils.clip_grad_norm_(parameters=model.vit_3d_ga1.parameters(), max_norm=10, norm_type=2)
        torch.nn.utils.clip_grad_norm_(parameters=model.vit_3d_ga2.parameters(), max_norm=10, norm_type=2)
        ...
        data_loader.desc = "[train {}] l:{:.3f},l2:{:.3f}, acc1:{:.3f},acc:{:.3f},acck1:{:.3f}".format(
            epoch,
